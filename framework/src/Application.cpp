...
void APIENTRY glErrorCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *msg, const void *data)
{
	...
	if (log != nullptr && type != GL_DEBUG_TYPE_OTHER && severity != GL_DEBUG_SEVERITY_NOTIFICATION )
	{
		//use stringstream to push things into string.
		std::stringstream sstm;
		sstm << errNum << ": GL Error Callback: " << "\n Error Source: " << strSource << "\n Error Type: " << strType << "\n Severity: " << strSeverity << "\n Message: " << msg << std::endl;
		log->addLog(LOG_LEVEL::LOG_ERROR, sstm.str().c_str());
		std::fstream file;
		file.open("GL_Error.log", std::ios_base::out);
